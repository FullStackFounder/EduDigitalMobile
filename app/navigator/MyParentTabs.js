/* eslint-disable react-native/no-inline-styles */
/* eslint-disable react/no-unstable-nested-components */
import React, {useState, useContext} from 'react';
import {View, Image, Dimensions} from 'react-native';
import {createBottomTabNavigator} from '@react-navigation/bottom-tabs';
import {Colors} from '../components/theme/color';
import {useSelector, useDispatch} from 'react-redux';
import Dash from '../screens/parents/Dash/Dash';
import Students from '../screens/parents/Students/Students';
import SchoolBills from '../screens/parents/SchoolBills/SchoolBills';
import Task from '../screens/parents/Task/Task';
import EnterPasscode from '../screens/parents/EnterPasscode/EnterPasscode';
import Menu from '../screens/parents/Menu/Menu';
import SchoolPayment from '../screens/parents/SchoolPayment/SchoolPayment';
import Academic from '../screens/parents/Academic/Academic';

const width = Dimensions.get('screen').width;
const height = Dimensions.get('screen').height;

const Tab = createBottomTabNavigator();

const MyParentTabs = () => {
  const {theme} = useSelector(state => state.theme);

  return (
    <Tab.Navigator
      screenOptions={{
        // BottomTabBarHeight:30,
        tabBarStyle: {
          position: 'absolute',
          backgroundColor: theme.tc,
          height: 70,
          marginHorizontal: 20,
          marginBottom: 20,
          borderRadius: 50,
          shadowColor: Colors.active,
          shadowOffset: {
            width: 0,
            height: 0,
          },
          shadowOpacity: 0.1,
          shadowRadius: 10,
          elevation: 5,
        },
        tabBarLabelStyle: {
          fontSize: 15,
        },
      }}>
      <Tab.Screen
        name="Siswa"
        component={Students}
        options={{
          tabBarShowLabel: false,
          tabBarIcon: ({focused, color}) => {
            return (
              <View
                style={{
                  height: 50,
                  width: 50,
                  borderRadius: 25,
                  backgroundColor: focused ? Colors.primary : theme.tc,
                  alignItems: 'center',
                  justifyContent: 'center',
                }}>
                <Image
                  source={require('../../assets/images/tab1.png')}
                  style={{
                    height: 30,
                    width: 30,
                    tintColor: focused ? Colors.secondary : theme.txt,
                  }}
                />
              </View>
            );
          },

          headerShown: false,
        }}
      />

      <Tab.Screen
        name="Tagihan"
        component={SchoolBills}
        options={{
          tabBarShowLabel: false,
          tabBarIcon: ({focused, color}) => {
            return (
              <View
                style={{
                  height: 50,
                  width: 50,
                  borderRadius: 25,
                  backgroundColor: focused ? Colors.primary : theme.tc,
                  alignItems: 'center',
                  justifyContent: 'center',
                }}>
                <Image
                  source={require('../../assets/images/studentbills/check.png')}
                  style={{
                    height: 30,
                    width: 30,
                    tintColor: focused ? Colors.secondary : theme.txt,
                  }}
                />
              </View>
            );
          },
          headerShown: false,
        }}
      />

      <Tab.Screen
        name="Pembayaran"
        component={SchoolPayment}
        options={{
          tabBarShowLabel: false,

          tabBarIcon: ({focused, color}) => {
            return (
              <View
                style={{
                  height: 50,
                  width: 50,
                  borderRadius: 25,
                  backgroundColor: focused ? Colors.primary : theme.tc,
                  alignItems: 'center',
                  justifyContent: 'center',
                }}>
                <Image
                  source={require('../../assets/images/studentbills/pay3.png')}
                  style={{
                    height: 28,
                    width: 28,
                    tintColor: focused ? Colors.secondary : theme.txt,
                  }}
                />
              </View>
            );
          },
          headerShown: false,
        }}
      />

      <Tab.Screen
        name="Akademik"
        component={Academic}
        options={{
          tabBarShowLabel: false,

          tabBarIcon: ({focused, color}) => {
            return (
              <View
                style={{
                  height: 50,
                  width: 50,
                  borderRadius: 25,
                  backgroundColor: focused ? Colors.primary : theme.tc,
                  alignItems: 'center',
                  justifyContent: 'center',
                }}>
                <Image
                  source={require('../../assets/images/academic/education.png')}
                  style={{
                    height: 28,
                    width: 28,
                    tintColor: focused ? Colors.secondary : theme.txt,
                  }}
                />
              </View>
            );
          },
          headerShown: false,
        }}
      />

      <Tab.Screen
        name="Profil"
        component={Menu}
        options={{
          tabBarShowLabel: false,
          tabBarIcon: ({focused, color}) => {
            return (
              <View
                style={{
                  height: 50,
                  width: 50,
                  borderRadius: 25,
                  backgroundColor: focused ? Colors.primary : theme.tc,
                  alignItems: 'center',
                  justifyContent: 'center',
                }}>
                <Image
                  source={require('../../assets/images/tab5.png')}
                  style={{
                    height: 30,
                    width: 30,
                    tintColor: focused ? Colors.secondary : theme.txt,
                  }}
                />
              </View>
            );
          },
          headerShown: false,
        }}
      />
    </Tab.Navigator>
  );
};

export default MyParentTabs;
